...
const Etiquette: React.FC<EtiqettesProps> = ({ onClick, className, text, direction, animate }) => {
    ...
    if (direction == "vertical")
        return (
            <div className={`transition-colors ${className} bg-[#262626] ${font} w-[75px] h-[240px] rounded-2xl justify-items-center`}>
        ...
        return (
            <button onClick={onClick} className={`flex transition-colors ${className} bg-[#262626] ${font} w-48 h-[75px] rounded-2xl items-center justify-center ${direction == "horizontal" ? "flex-row" : ""}`} role="button" aria-label={text}>
