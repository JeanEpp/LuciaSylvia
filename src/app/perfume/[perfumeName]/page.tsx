...
export default async function Page({ params }: { params: { perfumeName: string } }) {
  const paramsLoaded = await params;
  const perfumes = await getPerfumeNames();
  const etiquette = perfumes.find((p: IEtiquette) => p.name === paramsLoaded.perfumeName) ?? null;

  return (
    <div id="Parfums" className="h-min:screen text-center justify-items-center pb-8 pt-20 content-center space-y-9">
      <h1 className={`text-5xl font-semibold text-balance text-white text-center sm:text-5xl font-carattere`}>{paramsLoaded.perfumeName ?? ""}</h1>
